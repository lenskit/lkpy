...
def test_user_stats(ml_ratings: pd.DataFrame, ml_ds: Dataset):
    ...
    assert np.all(stats['count'] == ml_ratings['userId'].value_counts().reindex(ml_ds.users.index))
    assert np.all(stats['user_count'] == ml_ratings['userId'].value_counts().reindex(ml_ds.users.index))
    assert np.all(stats['rating_count'] == ml_ratings['userId'].value_counts().reindex(ml_ds.users.index))

    assert stats['mean_rating'].values == approx(ml_ratings.groupby('userId')['rating'].mean().reindex(ml_ds.users.index).values)
    assert np.all(stats['first_time'] == ml_ratings.groupby('userId')['timestamp'].min().reindex(ml_ds.users.index))
    assert np.all(stats['last_time'] == ml_ratings.groupby('userId')['timestamp'].max().reindex(ml_ds.users.index))
