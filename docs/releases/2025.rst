...
- Improve SLIM recommendation performance (:pr:`1140`, backported in :pr:`1141`)
- Backport type-checker fixes for NumPy 2.5
...
- Add SLIM (:class:`~lenskit.knn.SLIM`) (:pr:`896`, backported in :pr:`1058`).
...
- Add EASE (:class:`~lenskit.knn.EASEScorer`) (:issue:`942`, :pr:`1011`, backported in :pr:`1015`).
- Added LightGCN support to FlexMF (:issue:`1019`, :pr:`1033`).
- Fix hyperparameter tuning for non-iterative scorers, and simplify configuration (:pr:`1014`, backported in :pr:`1015`).
- Improve performance of :meth:`~lenskit.data.RelationshipSet.co_occurrences` (:issue:`970`, :pr:`1008`, :pr:`1010`).
...
- Put LightGCN in eval mode after training (:pr:`975`)
...
- Fix :issue:`956`: division by zero in NDCG (:pr:`957`).
...
- Fix a small bug in :meth:`lenskit.data.ItemListCollection.from_df` that duplicated
  user or session ID columns (:pr:`952`).
- Fixed :class:`~lenskit.data.ItemList`'s ``__getitem__`` to work correctly with
  negative slice bounds.
- Updated common pipelines to use the new
  :class:`~lenskit.basic.TrainingItemCandidateSelector` instead of the
  deprecated ``UnratedTrainingItemCandidateSelector``.  The behavior of these
  pipelines in the standard user-based recommendation configuration is
  unchanged, but they will now transparently work with other sources of query
  items (:pr:`953`).
...
- Add new :meth:`~lenskit.data.ItemList.concat` and
  :meth:`~lenskit.data.ItemList.update` methods to
  :class:`~lenskit.data.ItemList` to make it easier to extend and update item
  lists (:pr:`927`).
...
- Add :class:`~lenskit.basic.TrainingItemCandidateSelector` to replace the
  Unrated and All candidate selectors with better configuration.
- New function :func:`~lenskit.metrics.ranking.rank_biased_precision` to expose
  the inner computations of RBP.
- Add new method :meth:`~lenskit.data.RelationshipSet.co_occurrences` to count
  entity co-occurrences in a dataset.

Deprecations
------------

- :class:`~lenskit.basic.UnratedTrainingItemsCandidateSelector` is deprecated in
  favor of :class:`~lenskit.basic.TrainingItemCandidateSelector` with default
  configuration.
- :class:`~lenskit.basic.AllTrainingItemsCandidateSelector` is deprecated in
  favor of :class:`~lenskit.basic.TrainingItemCandidateSelector` with suitable
  configuration.
...
- Removed fixed defaults for ``row_entity`` and ``col_entity`` on
  ...
  for code working on standard interactions.
