...
- 🐞 Fixed bug in :meth:`lenskit.data.ItemList.top_n` that returned ordered lists with incorrect ranks (:issue:`1224`, :pr:`1221`, :pr:`1226`).
...
- Added diagnostics for power measurement to ``lenskit doctor`` (:issue:`1186`, :pr:`1189`).
- Added AMBAR dataset import (:issue:`1142`).
- Simplified missing history warnings (:issue:`804`, :pr:`1209`, first contribution by :user:`appleweiping`).
- Added inference-time mean pooling for new user activity to FlexMF (:issue:`934`, :pr:`1201`)
- Several test and CI refactors, including managing code policies with ``prek``/``pre-commit`` instead of ``hk``.
- Update Rust and Python dependencies
...
Third release in 2026, with more feature updates and bug fixes.  A full list of
fixed issues and merged PRs is on `milestone 2026.3`_.
...
- Added support for boolean and categorical hyperparameters to the tuner (:pr:`1139`).
- Added ``"pow2"`` scale for integer hyperparameters to select powers of 2 (:pr:`1144`).
- Optuna now includes the default hyperparameters as the first ones tested (:pr:`1145`).
- Moved configuration file schemas to :mod:`lenskit.schemas` (:pr:`1148`).
...
- Fixed correct running of CLI when CLI plugins cannot be imported.
...
- Added JSON export to :class:`~lenskit.data.ItemList` and
  :class:`~lenskit.data.ItemListCollection` (:pr:`1089`).
- Added ``--releated-items`` and ``--json`` options to the ``lenskit recommend``
  command (:pr:`1087`).
- Added a new scorer, :class:`lenskit.knn.AssociationScorer`, implementing
  conditional probability and lift-based association rule recommendations for
  both personalized and related-item recommendations (:issue:`930`, :pr:`1086`).
- Fixed a bug in pipeline merging that required derived pipelines to specify
  component classes in order to configure component options (:issue:`1097`,
  :pr:`1099`).
- Added a ``lenskit pipeline expand`` command to render a pipeline configuration
  to complete JSON for debugging / inspection (:issue:`1098`, :pr:`1099`).
- Added a new ``lenskit pipeline diagram`` command to render a pipeline configuration
  to a Mermaid diagram for visualization (:pr:`1100`).
- Re-add support for Pandas 2.2 to improve Google Colab support (:issue:`1103`,
  :pr:`1104`).
- Handle parameterized types for component inputs (:issue:`1101`, :pr:`1102`).

PRs and issues are listed in `milestone 2026.2`_.

.. _milestone 2026.2: https://github.com/lenskit/lkpy/issues?q=is%3Aclosed%20milestone%3A%222026.2%22
