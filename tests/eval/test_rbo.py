...
from lenskit.metrics.reranking import RBO


def test_rbo_empty_reference():
    ref = ItemList()
    rerank = ItemList([1, 2, 3], ordered=True)
    assert call_metric(RBO, ref, rerank) == 0.0
...
def test_rbo_perfect_match():
    ...
    score = call_metric(RBO, ref, rerank, k=5, p=0.9)
    assert score > 0.89
    assert score <= 1.0
...
def test_rbo_no_overlap():
    ...
    assert call_metric(RBO, ref, rerank, k=3, p=0.9) == approx(0.0)
...
def test_rbo_k_smaller_than_list():
    ...
    # consider only top 3
    score = call_metric(RBO, ref, rerank, k=3, p=0.9)
    assert 0 <= score <= 1
